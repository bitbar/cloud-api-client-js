...
api.me().runs().config().post().jsonData(config).send().then((response) => {
  console.log(response.status, response.data.runAvailable); // runAvailable tells if config is ok
});
